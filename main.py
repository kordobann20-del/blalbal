...
# Файлы для сохранения данных (чтобы на Railway ничего не пропадало)
...
def back_to_main(message):
    ...
    bot.send_message(message.chat.id, "Ок.", reply_markup=main_menu(message.from_user.username))
...
def handle_all_messages(message):
    global cards # ОБЯЗАТЕЛЬНО В НАЧАЛЕ ФУНКЦИИ
    ...
    if user_text == "Крутить карту":
        now = time.time()
        if not cards: return bot.send_message(message.chat.id, "⚠️ Карт нет.")
        if u_id in user_cooldowns and now - user_cooldowns[u_id] < 900:
            left = int((900 - (now - user_cooldowns[u_id])) / 60)
            return bot.send_message(message.chat.id, f"⏳ Жди {left} мин.")
        ...
        if user_text == "Добавить карту":
            ...
            bot.send_message(message.chat.id, "Название карты:")
        elif user_text == "Удалить карту":
            states[u_id] = {'action': 'delete'}
            bot.send_message(message.chat.id, "Какую карту удалить (название):")
        elif user_text == "Изменить карту":
            states[u_id] = {'action': 'edit_search'}
            bot.send_message(message.chat.id, "Название для изменения:")
            ...
            if state['action'] == 'delete':
                ...
                save_data()
                bot.send_message(message.chat.id, f"✅ Карта '{user_text}' удалена (если была).", reply_markup=admin_menu())
                del states[u_id]
                ...
                else: bot.send_message(message.chat.id, "Не найдена.")
                ...
                save_data()
                bot.send_message(message.chat.id, "✅ Готово!", reply_markup=admin_menu())
                del states[u_id]
